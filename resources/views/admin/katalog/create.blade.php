    ...
    <div class="max-w-5xl mx-auto">
        <form action="{{ route('admin.katalog.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" x-data="componentsForm()">
            ...
            <div class="bg-white p-8 border border-gray-200 rounded-sm shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary">
                    </div>
                    <div>
                        <label class="block font-semibold mb-2 text-gray-700">Series / Anime</label>
                        <input type="text" name="series" value="{{ old('series') }}" required class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary">
                    </div>
                    <div>
                        <label class="block font-semibold mb-2 text-gray-700">Ukuran (Size)</label>
                        <input type="text" name="size" value="{{ old('size') }}" required class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary" placeholder="S, M, L, XL, All Size">
                    </div>
                    <div>
                        <label class="block font-semibold mb-2 text-gray-700">Harga Sewa per Hari (Rp)</label>
                        <input type="number" name="base_price" value="{{ old('base_price') }}" required min="0" class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary">
                    </div>
                    <div>
                        <label class="block font-semibold mb-2 text-gray-700">Harga Uang Jaminan / Deposit (Rp)</label>
                        <input type="number" name="deposit_price" value="{{ old('deposit_price', 0) }}" required min="0" class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary">
                    </div>
                    <div>
                        <label class="block font-semibold mb-2 text-gray-700">Gambar Kostum</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" required class="w-full border-gray-300 rounded-sm p-3 focus:ring-0 focus:border-primary">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
                    ...
                    <template x-for="(comp, index) in components" :key="comp.id">
                        <div class="flex items-start gap-4 p-4 border border-gray-200 rounded-sm bg-gray-50">
                            <div class="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block font-semibold text-gray-700 text-sm mb-1">Nama Komponen (cth: Wig, Sepatu)</label>
                                ...
                                <div>
                                    <label class="block font-semibold text-gray-700 text-sm mb-1">Data QR Code (opsional)</label>
                                    <div class="relative">
                                        <input type="text" x-bind:name="`components[${index}][barcode]`" x-model="comp.barcode" @keydown.enter.prevent="comp.scanned = true; $event.target.blur()" class="w-full border-gray-300 rounded-sm p-2 focus:ring-0 focus:border-primary text-sm pr-8" placeholder="Tembak QR Code di sini">
                                ...
                                <div class="md:col-span-2">
                                    <label class="block font-semibold text-gray-700 text-sm mb-1">Gambar Komponen <span class="text-red-500">*</span></label>
                                    <input type="file" x-bind:name="`components[${index}][image]`" required accept="image/png, image/jpeg, image/jpg" class="w-full border border-gray-300 rounded-sm p-2 text-sm bg-white file:mr-3 file:py-1 file:px-3 file:rounded-sm file:border-0 file:text-xs file:font-bold file:bg-gray-200 file:text-gray-800 hover:file:bg-gray-300">
                                </div>
                            </div>
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit" class="bg-primary text-gray-900 font-bold px-8 py-3 rounded-sm hover:bg-[#E5A5B0] transition-colors shadow-sm">Simpan Kostum</button>
            </div>
        </form>
    </div>
