        ...
        @else
            <div class="space-y-8">
                @foreach($bookings as $booking)
                    <div class="bg-white border-2 border-gray-900 shadow-[8px_8px_0_0_rgba(17,24,39,1)] p-6 md:p-8 flex flex-col md:flex-row gap-8 transition-transform hover:-translate-y-1 hover:shadow-[12px_12px_0_0_rgba(17,24,39,1)]">
                        <!-- Info Kostum -->
                        <div class="flex-grow">
                            <div class="flex flex-wrap items-center gap-4 mb-4">
                                <h3 class="text-2xl font-black text-gray-900 uppercase">{{ $booking->costume->name }}</h3>
                                <!-- Status Badge -->
                                @php
                                ...
                                @endphp
                                <span class="text-xs font-black px-3 py-1.5 border-2 {{ $statusColor }} uppercase tracking-widest shadow-[2px_2px_0_0_rgba(17,24,39,1)]">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                                <div>
                                    <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Tanggal Sewa</p>
                                ...
                                <div>
                                    <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Total Biaya</p>
                                    <p class="font-black text-gray-900 text-xl bg-primary px-3 py-1 inline-block border-2 border-gray-900 shadow-[2px_2px_0_0_rgba(17,24,39,1)]">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</p>
                                </div>
                            </div>

                                <div class="mb-6">
                                    <p class="text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Alamat Pengiriman</p>
                                    <p class="font-medium bg-gray-50 p-4 border-2 border-gray-900 text-sm text-gray-900">
                                ...
                                @if($booking->status === 'Sedang Dikirim')
                                    <form action="{{ route('orders.received', $booking->id) }}" method="POST" x-data @submit.prevent="$dispatch('open-confirm', { title: 'Konfirmasi Pesanan Diterima', message: 'Apakah Anda yakin pesanan sudah sampai?', form: $el })">
                                        @csrf
                                        <button type="submit" class="bg-secondary text-gray-900 font-black px-6 py-3 border-2 border-gray-900 hover:bg-white transition-colors shadow-[4px_4px_0_0_rgba(17,24,39,1)] text-sm uppercase tracking-wider">
                                ...
                                @if($booking->status === 'Sedang Dirental')
                                    <form action="{{ route('orders.returnShipping', $booking->id) }}" method="POST" class="mt-6 border-t-2 border-gray-900 pt-6" x-data @submit.prevent="$dispatch('open-confirm', { title: 'Konfirmasi Pengembalian', message: 'Anda yakin ingin menyimpan data pengembalian?', form: $el })">
                                        @csrf
                                        <div class="mb-4">
                                            <label for="return_shipping_receipt" class="block text-xs font-black text-gray-900 uppercase tracking-widest mb-2">Nomor Resi Pengembalian</label>
                                            <input type="text" name="return_shipping_receipt" id="return_shipping_receipt" class="w-full bg-white border-2 border-gray-900 px-4 py-3 text-sm focus:ring-0 focus:outline-none focus:border-primary focus:shadow-[4px_4px_0_0_rgba(255,46,99,1)] transition-shadow" placeholder="Masukkan nomor resi ekspedisi" required>
                                        </div>
                                        <button type="submit" class="bg-gray-900 text-white font-black px-6 py-3 border-2 border-gray-900 hover:bg-primary hover:text-gray-900 transition-colors shadow-[4px_4px_0_0_rgba(17,24,39,1)] text-sm uppercase tracking-wider w-full sm:w-auto">
                        ...
                        <div class="md:w-1/3 border-t-2 md:border-t-0 md:border-l-2 border-gray-900 pt-6 md:pt-0 md:pl-8 bg-gray-50 md:bg-transparent -mx-6 -mb-6 md:m-0 p-6 md:p-0">
                            <p class="font-black text-gray-900 text-sm mb-4 uppercase tracking-widest">Komponen Kostum</p>
