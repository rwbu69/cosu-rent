...
<div class="mb-10">
    ...
    <form action="{{ route('address.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="address_line" id="address_line" rows="3" required class="w-full border border-gray-200 rounded-md p-3 focus:ring-0 focus:border-primary font-medium" placeholder="Contoh: Jl. Sudirman No. 1, Jakarta Pusat, 10110">{{ old('address_line') }}</textarea>
            @error('address_line')
                <p class="text-red-600 font-bold mt-2 text-sm">{{ $message }}</p>
            @enderror
        </div>
        <button type="submit" class="bg-gray-900 text-white font-bold px-6 py-3 border border-gray-200 rounded-md hover:bg-primary hover:text-gray-900 transition-colors uppercase">
            Simpan Alamat
        </button>
    </form>
</div>
